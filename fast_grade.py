...
if "annotated_df.pckl" in os.listdir(f"{bpath}"):
    ...
    Annotated_DF = pd.DataFrame.from_dict(
        {key: [""] for key in "image_id elements date_added".split(" ")}
    )
...
def get_inst_mask_from_point(pic_id: int, x_loc: int, y_loc: int):
    ...
    img_name = "{}/{}".format(
        ade_index["folder"][pic_id], ade_index["filename"][pic_id]
    )
...
def get_image(pic_id: int):
    ...
    img = np.where(
        np.repeat(segment_lines[:, :, np.newaxis], 3, 2).astype(bool),
        np.stack([segment_lines * 255, segment_lines * 255, segment_lines * 0], 2),
        img.astype("float64"),
    )
    success, im = cv2.imencode(".png", img)
    img_shape = img.shape
...
async def add_image_subgraph(info: Request):
    ...
    new_graph_data = json.loads(new_graph_data)
    ic(new_graph_data)
    new_graph = new_graph_data["graph"]
    return {"status": "SUCCESS", "sizeOfGraph": len(new_graph)}
