...
class Image_Maps:
    ...
    def add_from_loc(self, loc):
        x = int(loc[0] * self.xscale)
        y = int(loc[1] * self.yscale)
        ent_val = self.instance_mask[x, y]
    ...
    def add_preview(self, idx):
        new_temp_indices = [
            ...
            if idx in i
            for j in i
            if j not in self.preview_indices
        ]
        if idx in new_temp_indices:
            new_temp_indices.remove(idx)

        new_temp_indices = (
            [new_temp_indices] if type(new_temp_indices) != list else new_temp_indices
        )
    ...
    def recalc_preview(self):
        adjacent_idx = [
            j
            for i in self.image_graph
            if all([ii in i for ii in self.entities])
            for j in i
        ]
    ...
    def save_group_state(self):
        ...
        jdump = {"graph": [int(i) for i in self.entities], "image_id": self.image_idx}
        json_ent = json.dumps(jdump)
        ic(json_ent)
        result = req.post(f"http://127.0.0.1:8000/graph/add", json=json_ent)
        self.entities = []
        self.preview_mask *= 0
...
def byte_to_plt(data, img_shape):
    gfig = np.frombuffer(data["image"], dtype=np.unit8).reshape(
        figure_data["img_shape"]
    )
...
def main(page: ft.Page):
    ...
    def add_item(e: ft.TapEvent):
        loc_x = int((e.local_x - 4))
        loc_y = int((e.local_y - 4))
    ...
    if "current_map_data" not in locals():
        ...
        w0, h0, _ = gfig.shape
        w = MAX_IMAGE_WIDTH if w0 > h0 else w0 * (MAX_IMAGE_HEIGHT / h0)
        h = MAX_IMAGE_HEIGHT if h0 > w0 else h0 * (MAX_IMAGE_WIDTH / w0)
        figsize_x = (w / h) * 10
        seg_fig, seg_ax = plt.subplots(figsize=(10, figsize_x))
        seg_fig.subplots_adjust(0, 0, 1, 1)
        current_map_data = Image_Maps(
            image=gfig,
        ...
        img_size = gfig.shape
        display_figure = MatplotlibChart(seg_fig, ref=image_mpl, expand=True)

    def save_and_switch(e):
        current_map_data.image_idx += 1
        figure_data = req.get(
            f"http://127.0.0.1:8000/images/outlines/{current_map_data.image_idx}"
        )
        figure_graph_data = req.get(
            f"http://127.0.0.1:8000/images/graphs/{current_map_data.image_idx}"
        )
        figure_instance_data = req.get(
            f"http://127.0.0.1:8000/images/instances/{current_map_data.image_idx}"
        )
        current_image_graphs = parse_rest_graphs(figure_graph_data)
        arr = np.frombuffer(figure_data.content, np.uint8)
        gfig = cv2.imdecode(arr, cv2.IMREAD_UNCHANGED)
        ...
        w = MAX_IMAGE_WIDTH if w0 > h0 else w0 * (MAX_IMAGE_HEIGHT / h0)
        h = MAX_IMAGE_HEIGHT if h0 > w0 else h0 * (MAX_IMAGE_WIDTH / w0)
        current_map_data.xscale = w0 / w
        current_map_data.yscale = h0 / h
        card.content.width = w
        card.content.height = h
        image_stack.width = MAX_IMAGE_WIDTH
        image_stack.height = MAX_IMAGE_HEIGHT
        card.width = w
        card.height = h
        display_figure.width = w
        display_figure.height = h
        figsize_x = (h / w) * 10
        seg_fig, seg_ax = plt.subplots(figsize=(10, figsize_x))
        seg_fig.subplots_adjust(0, 0, 1, 1)
        seg_ax.imshow(current_map_data.image)
    ...
    image_stack = ft.Stack([card], width=MAX_IMAGE_WIDTH, height=MAX_IMAGE_WIDTH)
    page.add(
        ft.Row(
            [
                ft.Column(
                ...
                ft.Column(
                    [
                        ft.ElevatedButton(
                            text="Save Group", on_click=save_new_group_state
                        ),
                        #     ft.ElevatedButton(text="Delete Selections"),
                        ft.ElevatedButton(
                            text="Next Image",
                            on_click=save_and_switch,
